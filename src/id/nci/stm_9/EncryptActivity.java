...
public class EncryptActivity extends SherlockFragmentActivity {
    ...
    /* Intents */
    public static final String ACTION_ENCRYPT = "id.nci.stm_9" + ".action." + "ENCRYPT";
...
//            menu.add(1, Id.menu.option.encrypt_to_clipboard, 0, mEncryptToClipboardString)
            menu.add(1, 0x21070014, 0, mEncryptToClipboardString)
                    .setShowAsAction(
                            MenuItem.SHOW_AS_ACTION_ALWAYS | MenuItem.SHOW_AS_ACTION_WITH_TEXT);
        }
        if (mEncryptEnabled) {
//            menu.add(1, Id.menu.option.encrypt, 1, mEncryptString).setShowAsAction(
            menu.add(1, 0x21070013, 1, mEncryptString).setShowAsAction(
                    MenuItem.SHOW_AS_ACTION_ALWAYS | MenuItem.SHOW_AS_ACTION_WITH_TEXT);
        }
...
//            mEncryptTarget = Id.target.file;
            mEncryptTarget = 0x21070003;
        } else {
//            mEncryptTarget = Id.target.email;
            mEncryptTarget = 0x21070002;
        }
        initiateEncryption();
    }
...
//            if (!encryptIt && mEncryptTarget == Id.target.file) {
            if (!encryptIt && mEncryptTarget == 0x21070003) {
...
//                Id.request.output_filename);
    ...
    private void encryptStart() {
        ...
        if (mMode.getCurrentView().getId() == R.id.modeSymmetric) {
            Log.d("stm-9", "Symmetric encryption enabled!");
            String passPhrase = mPassPhrase.getText().toString();
...
//                        + ")"), };
        		new Choice(0x21070001, getString(R.string.choice_none) + " ("
        				+ getString(R.string.compression_fast) + ")"),
        		new Choice(CompressionAlgorithmTags.ZIP, "ZIP (" + getString(R.string.compression_fast) + ")"),
        		new Choice(CompressionAlgorithmTags.ZLIB, "ZLIB (" + getString(R.string.compression_fast) + ")"),
        		new Choice(CompressionAlgorithmTags.BZIP2, "BZIP2 (" + getString(R.string.compression_very_slow)
        				+ ")"), };
...
//        startActivityForResult(intent, Id.request.public_keys);
        startActivityForResult(intent, 0x00007001);
    }

    private void selectSecretKey() {
        Intent intent = new Intent(this, SelectSecretKeyActivity.class);
//        startActivityForResult(intent, Id.request.secret_keys);
        startActivityForResult(intent, 0x00007002);
    }
